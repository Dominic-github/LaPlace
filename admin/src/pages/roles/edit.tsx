...
import { useOne, useUpdate, useNavigation, useCustom } from "@refinedev/core";
...
export const RoleEdit = () => {
  ...
  // Fetch role data
  const { data: roleQuery, isLoading } = useOne({
    resource: "roles",
    id: id as string,
  });
  ...
  useEffect(() => {
    if (roleData) {
      ...
      const permIds = roleData.permissions?.map((p: any) => p.permissionId || p.permission?.id) || [];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    update(
      ...
      {
        onSuccess: () => {
          showMessage("success", "Cập nhật vai trò thành công!");
        },
        onError: (error: any) => {
              ...
              <CardContent className="space-y-6">
                {Object.keys(permissionsByModule).length === 0 ? (
                  <p className="text-muted-foreground text-center py-4">
                    Chưa có quyền nào được tạo. Chạy <code>npm run seed:roles</code> để tạo.
                  </p>
                ) : (
