...
export const metadata: Metadata = { 
  ...
  description: 'Cho thuê căn hộ, chung cư mini, studio đầy đủ nội thất' 
};

export default async function CanHoPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  try {
    const res = await accommodationApi.getAll({ type: 'can-ho', page, limit: 12 });
    if (res.data?.data) {
  ...
  return (
    <div className="container-page pb-16">
      <Breadcrumbs items={[{ label: 'Căn hộ' }]} />
      <div className="mb-8 mt-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Căn hộ cho thuê</h1>
        <p className="text-gray-500">Căn hộ dịch vụ, chung cư mini, studio full nội thất chất lượng, giá tốt</p>
      </div>

      <div className="flex flex-wrap gap-2 mb-6 p-4 bg-gray-50 rounded-xl">
        <Link href="/tim-phong?type=can-ho" className="btn-primary btn-sm px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {listings.length > 0 ? (
          ...
          <div className="col-span-full py-20 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            <p className="text-gray-500 mb-4">Hiện chưa có căn hộ nào phù hợp.</p>
          </div>
        )}
      </div>

      <Pagination currentPage={page} totalPages={pagination.totalPages} baseUrl="/can-ho" />
    </div>
  );
}
