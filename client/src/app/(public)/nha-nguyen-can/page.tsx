...
export const metadata: Metadata = { 
  ...
  description: 'Cho thuê nhà nguyên căn, biệt thự, nhà phố cho gia đình và kinh doanh' 
};

export default async function NhaNguyenCanPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  try {
    const res = await accommodationApi.getAll({ type: 'nha-nguyen-can', page, limit: 12 });
    if (res.data?.data) {
      listings = res.data.data.items;
      pagination = res.data.data.pagination;
    }
  } catch (error) {
    console.error('Error fetching Nha nguyen can:', error);
  }

  return (
    <div className="container-page pb-16">
      <Breadcrumbs items={[{ label: 'Nhà nguyên căn' }]} />
      <div className="mb-8 mt-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Nhà nguyên căn chờ đón bạn</h1>
        <p className="text-gray-500">Phù hợp cho gia đình hoặc nhóm bạn, đầy đủ tiện nghi</p>
      </div>

      <div className="flex flex-wrap gap-2 mb-6 p-4 bg-gray-50 rounded-xl">
        <Link href="/tim-phong?type=nha-nguyen-can" className="btn-primary btn-sm px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {listings.length > 0 ? (
          ...
          <div className="col-span-full py-20 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            <p className="text-gray-500 mb-4">Hiện chưa có nhà nguyên căn nào phù hợp.</p>
          </div>
        )}
      </div>

      <Pagination currentPage={page} totalPages={pagination.totalPages} baseUrl="/nha-nguyen-can" />
    </div>
  );
}
