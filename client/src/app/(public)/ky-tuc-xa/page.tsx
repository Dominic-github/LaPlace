...
export const metadata: Metadata = { 
  ...
  description: 'Cho thuê ký túc xá, sleepbox, phòng trọ ghép giá siêu tiết kiệm' 
};

export default async function KyTucXaPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  try {
    const res = await accommodationApi.getAll({ type: 'ky-tuc-xa', page, limit: 12 });
    if (res.data?.data) {
      listings = res.data.data.items;
      pagination = res.data.data.pagination;
    }
  } catch (error) {
    console.error('Error fetching Ky tuc xa:', error);
  }

  return (
    <div className="container-page pb-16">
      <Breadcrumbs items={[{ label: 'Ký túc xá' }]} />
      <div className="mb-8 mt-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Ký túc xá & Sleepbox</h1>
        <p className="text-gray-500">Giải pháp lưu trú giá sinh viên, tiết kiệm chi phí</p>
      </div>

      <div className="flex flex-wrap gap-2 mb-6 p-4 bg-gray-50 rounded-xl">
        <Link href="/tim-phong?type=ky-tuc-xa" className="btn-primary btn-sm px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {listings.length > 0 ? (
          ...
          <div className="col-span-full py-20 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            <p className="text-gray-500 mb-4">Hiện chưa có ký túc xá nào phù hợp.</p>
          </div>
        )}
      </div>

      <Pagination currentPage={page} totalPages={pagination.totalPages} baseUrl="/ky-tuc-xa" />
    </div>
  );
}
